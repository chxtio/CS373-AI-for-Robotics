...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # Expand omega and xi by 1 new position
        arr = [0, 1]
        for i in range(4, dim+2):
            arr.append(i)
##        print('\nExpand index array: ', arr)
            
        Omega = Omega.expand(dim+2, dim+2, arr, arr)
        Xi = Xi.expand(dim+2, 1, arr, [0])

##        Omega.show('Expanded Omega: \n')
##        Xi.show('Expanded  Xi:    \n')
...
##        Omega.show('Omega before take: \n')
##        Xi.show('Xi before take:    \n')

        # Factor out previous pose
        newlist = range(2, len(Omega.value))
##        print('take index array: ', newlist)
        B = Omega.take([0, 1])
...
##Landmarks:  [[34, 96], [64, 19], [82, 11], [61, 67], [18, 34]]
##Robot: [x=61.66481 y=86.13761]
##
##Estimated Pose(s):
##    [58.485, 84.223]
##
