...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ### ENTER CODE HERE
        px, py = estimate[0], estimate[1]
        x1, y1 = spath[index][0], spath[index][1]
        dx, dy = spath[index+1][0] - x1, spath[index+1][1] - y1
        drx, dry = px - x1, py - y1
        
        # U is the robot estimate projected onto path segment
        u = (drx * dx + dry * dy) / (dx * dx + dy * dy)
        # CTE is estimate projected onto the normal of path segment
        cte = (dry * dx - drx * dy) / sqrt(dx**2 + dy**2)
        # Pick next path segment
        if u > 1.0:
            index += 1
